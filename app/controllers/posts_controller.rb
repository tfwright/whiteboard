class PostsController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      format.html
      format.json do 
        render :json => {:posts => @posts.map{|p| p.attributes.merge(:unviewed => p.new_for?(current_user), "body" => truncate(p.body, :length => 600), "url" => course_post_url(@current_course, p), "reply-count" => p.replies.count, :author => p.author)}, 
