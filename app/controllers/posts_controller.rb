class PostsController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      format.html
      format.json do 
        render :json => {:posts => @posts.map{|p| p.attributes.merge(:unviewed => p.author != current_user && !View.exists?(:user_id => current_user.id, :post_id => p.id), "body" => truncate(p.body, :length => 600), "url" => course_post_url(@current_course, p), "reply-count" => p.replies.count, :author => p.author)}, 
